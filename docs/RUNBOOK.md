...
npx tsx scripts/db-query.mjs "SELECT decision,decision_code,settled,tx_id FROM payment_requests ORDER BY created_at DESC LIMIT 3;"
...
npx tsx scripts/db-query.mjs "SELECT intent_id,status,consumed_at FROM intents;"
```
...
npx tsx scripts/db-query.mjs "SELECT COUNT(*) FROM payment_requests WHERE decision='DENY' AND settled=1;"  # 0
```
